...
if(isset($_GET['action'])) {
    if($_GET['action'] == 'delete') {
        if(isset($_GET['id'])) {
          ...
          $invalids->id = $_GET['id'];
          $invalids->admin_id = trim($_GET['admin_id']);
          $invalids->date_at = trim($_GET['date_at']);
          if($invalids->deleteInvalid()) {
            header('Location: invalids.php?alert=success');
            $success = true;
          } else {
            $error = true;
          } 
...
$invalid = new Invalid();
$invalid->admin_id = $user_logged_id;
$getInvalids = $invalid->getInvalids();
?>
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header">
          <h3 class="card-title">Lista de Inválidos</h3>

          <a class="btn btn-info btn-sm" href="new-invalid.php" style="float: right;">
            ...
            Novo Inválido
                  ...
                  <?php if(!empty($getInvalids)) { ?>
									<?php foreach($getInvalids as $row) { 
                    $getDate = getMonthBR(date("m", strtotime($row->date_at))) . '/'. date("Y", strtotime($row->date_at));
                    ?>
                    <tr>
                        <td><?php echo $getDate; ?></td>                     
                        <td>$ <?php echo number_format($row->invalid, 2, ',', '.'); ?></td>        
                        <td><?php echo getFormatDate($row->created_at, 1); ?></td>
                        <td style="text-align:center;">
                            <a class="btn btn-info btn-sm" href="edit-invalid.php?id=<?php echo $row->id ?>">
